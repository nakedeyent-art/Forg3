...
_Last updated: 2026-07-30 UTC (Codex session: Forg3 App Store review resubmission, iOS build 5)_
...
- `npm run appstore:products` configured Apple subscription group/product localizations, review screenshots, availability, and Apple-equalized pricing. Apple rejected the prior Forg3 review package because the review/demo account could land in an active creator entitlement state and hide the Pro/Business StoreKit purchase cards after sign-in. The native billing UI now keeps StoreKit plan cards visible for creator/review accounts. Build `5` is attached to iOS version `1.0`; the app version plus `Forg3 Pro` and `Forg3 Business` are `WAITING_FOR_REVIEW` in review submission `14bd210c-daa7-4e0f-9585-ca4e3090a2d3` as of `2026-07-30T09:42:09Z`.
