...
_Last updated: 2026-08-02 UTC (Codex session: Forg3 App Store build 6 submitted)_

## Current live state

| Thing | Value |
| --- | --- |
| Staging URL | **https://forg3.nak3deye.com** live. Cloudflare DNS has a proxied A record `forg3.nak3deye.com -> 129.153.239.91`; Cloudflare SSL/TLS mode is `Full`. |
| Compute | OCI container instance `forg3-staging-v15` (us-ashburn, `CI.Standard.A1.Flex` 1 OCPU / 2 GB), containers: `forg3-app` + `caddy`, public IP `129.153.239.91` |
...
| Image | `ghcr.io/nakedeyent-art/forg3:b4333ba1f94c30f44e20fa266f92c34004dc751d` (multi-arch, published by CI on every push to main) |
| Repo | github.com/nakedeyent-art/Forg3, default branch `main`, CI green on commit `b4333ba Align document signing release copy`; `publish-ghcr` completed in CI run `29631698914` |
| Old staging | v7/v8/v9/v10/v11/v12/v13 are deleted after quota cleanup. v14 remains active as temporary rollback but is no longer the Cloudflare DNS target. Current live DNS target is v15 only. |
...
- The v15 app container boots with Postgres persistence and encrypted object storage; direct origin `/api/health` passes at `129.153.239.91`.
...
- Production monitor: Cloudflare proxied DNS for `forg3.nak3deye.com` points at v15 (`129.153.239.91` origin), public `/api/health` passes over HTTPS, public bundle contains the multi-file document upload UI, and `npm run monitor:production` passes.
...
- `npm run appstore:products` configured Apple subscription group/product localizations, review screenshots, availability, and Apple-equalized pricing. Apple rejected iOS version `1.0` build `5` on `2026-07-31` because Sign in with Apple/Google errored and the subscription purchase attempt errored on iPad Air 11-inch (M3), iPadOS 26.5.2. The corrective build hides native Google/Apple provider buttons unless native provider auth is explicitly enabled and real-device tested, keeps StoreKit plan cards visible for creator/review accounts, and verifies Apple sandbox/review transactions against the correct App Store Server API endpoint before granting entitlement. Build `6` was uploaded, processed as `VALID`, attached to iOS version `1.0`, and submitted with `Forg3 Pro Monthly` / `Forg3 Business Monthly` in review package `50a817fa-7921-4679-8699-cb8219dee0b7` on `2026-08-02`; App Store Connect now reports `WAITING_FOR_REVIEW`.
...
- `npm run play:internal -- .deploy/mobile/forg3-1.0-build4-api36-play-release-20260729T174006Z.aab` and `npm run play:track` uploaded/promoted Android versionCode `4` to Play tracks `internal` and `alpha` with release status `completed` through the Google Play Developer API on July 29, 2026.
...
- Play closed testing Alpha was configured and submitted through Play Console on July 28, 2026 at 23:13 EDT: release `3 (1.0)`, 177 countries/regions, tester lists `Forg3 Internal Testers` and `The Daily Edge Android List`, and 14 changes sent for review. Android Publisher API readback at `2026-07-29T03:14:36Z` showed `alpha=3 (1.0):completed`; Play dashboard showed update status `In review`, `0 testers currently opted-in`, and `Apply for production` disabled at 23:16 EDT. On July 29, 2026, the Google Play target API warning was fixed by raising Android compile/target SDK to API 36, building signed `versionCode 4`, and uploading `.deploy/mobile/forg3-1.0-build4-api36-play-release-20260729T174006Z.aab` to Internal and Alpha; API readback at `2026-07-29T17:47:32Z` shows `internal=Forg3 1.0 (4):completed` and `alpha=Forg3 1.0 (4):completed`. Closed-test join links: Android `https://play.google.com/store/apps/details?id=com.forg3.sign`; web opt-in `https://play.google.com/apps/testing/com.forg3.sign`.
...
- Current signed Android release AAB: `.deploy/mobile/forg3-1.0-build4-api36-play-release-20260729T174006Z.aab` (SHA-256 `48a05a53106e122df590a95a685ea37a722ae0ac526c25ecb6459596d1e1f656`)
...
2. Real-device iOS/Android QA (priority 9): iOS build `6` is submitted and waiting for App Review; Android signed AAB versionCode `4` is uploaded to Play Alpha closed testing. Full runtime QA is still pending for paid purchase/restore/manage flows. Required final test path: email login, device 2FA, upload PDF/Word/Excel/PowerPoint, send email link, recipient-only access, signing, signed package download, native purchase, restore purchase, and manage subscription. In-app account deletion exists at `#/settings`, and the public deletion page exists at `#/account-deletion`.
3. Native billing (priority 7): server receipt-verification and webhook plumbing exists for Apple App Store Server API and Google Play Developer API, and native StoreKit 2 / Play Billing bridges now exist in the Capacitor shells. Apple client StoreKit payloads are no longer trusted directly; the server verifies through App Store Server API before entitlement and now uses the transaction environment to prefer sandbox vs production correctly. Apple Pro/Business subscriptions are resubmitted and waiting for review with build `6`. Google Play Pro/Business products are active, Android internal release versionCode `4` is uploaded, and closed testing Alpha release `Forg3 1.0 (4)` is active/completed. Remaining work is Apple review monitoring, sandbox purchase tests, tester opt-in/install confirmation, Google's 12-tester/14-day closed-test production gate, and the final per-signature billing model decision. Native mobile currently shows Pro/Business only; Pay Per Signature is hidden until the usage model is store-compliant. Product IDs are defined in `server/index.ts` (`com.forg3.sign.*` / `forg3_*`). See `docs/STORE_BILLING_IMPLEMENTATION.md`.
4. iOS TestFlight/App Store status: App Store Connect app/version exists for `com.forg3.sign`; build `5` was rejected on `2026-07-31` in review submission `14bd210c-daa7-4e0f-9585-ca4e3090a2d3`. Build `6` is uploaded, `VALID`, attached, and submitted in review package `50a817fa-7921-4679-8699-cb8219dee0b7`; App Store Connect reports iOS version `1.0` as `WAITING_FOR_REVIEW`, and both launch subscriptions report `WAITING_FOR_REVIEW`. App Review detail/contact, review notes, demo account, age rating, app metadata, free download pricing, App Privacy labels, and export compliance are configured; build `6` reports `usesNonExemptEncryption=false`, and the iOS shell sets `ITSAppUsesNonExemptEncryption=false`.
...
7. External console gates: App Store Connect app metadata, App Review detail, age rating, App Privacy, pricing, content rights, copyright, primary category, and export compliance are configured; build `6` and the first two subscriptions are waiting for Apple review. Google Play internal and closed testing Alpha have accepted `Forg3 1.0 (4)` with API 36 target SDK. Add at least 3 more eligible testers, get at least 12 testers opted in, keep them opted in for 14 continuous days, then apply for production access. Tester opt-in/device install, sandbox purchase verification, and native Google/Apple signup real-device verification are still pending.
